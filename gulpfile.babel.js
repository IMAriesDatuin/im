...
export const robots = () => {
	...
	return gulp.src([paths.build + '**/*.{html,php}',
					 '!' + paths.build + config.html.dialog.output + '**/*'])
			   .pipe(robotstxt({
...
// VARS
const partials = { part01: 'framework.js', part02: 'core.js', part03: 'main.js' };
...
export const lintjs = () => {
	...
	return gulp.src( config.js.main.files.map( function(base) { return paths.source + paths.mainJS + base } ), {allowEmpty: true} )
...
export const corejs = () => {
	...
	console.log(`Preparing core JS files...`);

	return gulp.src( config.js.core.files.map( function(base) { return paths.source + paths.coreJS + base } ), {allowEmpty: true} )
			   .pipe(_if( !production && config.compiler.cacheBuild, cache('corejs') ))
			   .pipe(_if( !production && config.compiler.rememberFiles, changed(paths.build + paths.scripts) ))
			   .pipe(removeCode({removeUnusedCode: true}))
			   .pipe(concat(partials.part02))
...
export const mainjs = () => {
	...
	return gulp.src( config.js.main.files.map( function(base) { return paths.source + paths.mainJS + base } ), {allowEmpty: true} )
			   .pipe(_if( !production && config.compiler.cacheBuild, cache('mainjs') ))
			   .pipe(_if( !production && config.compiler.rememberFiles, changed(paths.build + paths.scripts) ))
			   .pipe(removeCode({removeUnusedCode: true}))
			   .pipe(concat(partials.part03))
			   //.pipe(babel(['@babel/preset-env']))
			   .pipe(gulp.dest( paths.build + paths.scripts ));
...
export const combinejs = () => {
	...
	console.log(`Compiling ${config.js.bundle}.js...`);

	return gulp.src([paths.build + paths.temp + 'modernizr.js',
					 paths.build + paths.scripts + partials.part01,
					 paths.build + paths.scripts + partials.part02,
					 paths.build + paths.scripts + partials.part03], {allowEmpty: true} )


			   // Generate sourcemaps.
			   .pipe(_if( config.js.sourcemaps, sourcemaps.init() ))
			   .pipe(concat(config.js.bundle + '.js'))
			   ...
			   .on('end', function () {
					...
					return deleteAssets([paths.build + paths.scripts + partials.part01,
						 				 paths.build + paths.scripts + partials.part02,
						 				 paths.build + paths.scripts + partials.part03
		 								], { allowEmpty: true });
...
export const vendorsjs = (done) => {
	...
	if ( config.js.vendors.allow ) {

		console.log(`Compiling ${config.js.vendors.bundle}.js...`);

		return gulp.src( paths.source + config.js.vendors.files, {allowEmpty: true} )
				   ...
				   // Generate sourcemaps.
				   .pipe(_if( config.js.sourcemaps, sourcemaps.init() ))
				   .pipe(concat(config.js.vendors.bundle + '.js'))
...
export const dialog = () => {
	...
	return gulp.src( paths.source + config.html.dialog.files )
			   .pipe(panini({
							 root: paths.source + config.html.dialog.files,
							 layouts: paths.source + config.html.layouts,
							 partials: paths.source + config.html.partials,
							 helpers: paths.source + config.html.helpers,
							 data: paths.source + config.html.data
			   ...
			   .pipe(_if( production, htmlMinify(htmlMinifyOptions) ))
			   .pipe(replaceString('&lt;br&gt;', '<br>'))
			   .pipe(noopener.warn())
			   .pipe(noopener.overwrite())
			   .pipe(gulp.dest( paths.build + config.html.dialog.output ));
...
export const minify = () => {
	...
	return gulp.src( paths.build + '**/*.{html,php}', { allowEmpty: true } )
			   .pipe(replaceString('siteVersion', config.site.version))
			   ...
			   .pipe(_if( !config.js.vendors.allow, removeCode({removeVendors: true}) ))
	...
	return gulp.src(paths.build + '**/*.{html,php}', { base: paths.build } )
			   ...
			   .pipe(inject(
					...
					gulp.src([paths.build + paths.scripts + '**/*.js',
							  ...
							  '!' + paths.build + paths.scripts + partials.part01,
							  '!' + paths.build + paths.scripts + partials.part02], {read: false, allowEmpty: true} ), {
			   ...
			   .pipe(inject(
					...
					gulp.src([paths.build + paths.scripts + config.js.vendors.bundle], {read: false, allowEmpty: true} ), {
...
export const sync = (done) => {
	...
	gulp.watch([paths.source + '**/*.js',
				...
				]).on('all', gulp.series(lintjs, frameworkjs, corejs, mainjs, combinejs, injectscripts), reload);


	gulp.watch(paths.source + config.js.vendors.files).on('all', vendorsjs, reload);
	...
	gulp.watch([//credentialsFile,
				...
				//paths.source + '{_data,_helpers,_layouts,_dialog,_partials}/**/*.{html,hbs,handlebars,json,yml}'
				//paths.source + '{'+config.html.layouts+','+config.html.partials+','+config.html.helpers+','+config.html.data+'}/**/*.{html,hbs,handlebars,json,yml}'
				]).on('all', gulp.series(html, dialog, meta, css, refresh, gulp.series(refresh, html, dialog, meta, injectwebfontloader, injectscripts, reload) ));
...
// TEST
gulp.task('test', gulp.series(clearcache, version, mode, backup, clear, html, dialog, injectwebfontloader, lintjs, vendorsjs, frameworkjs, corejs, mainjs, combinejs, css, injectscripts, move, appicons, meta, zipassets, a11ycheck, optimizeassets, clean, sync));


// BUILD
gulp.task('build', gulp.series(version, clear, revconfig, html, dialog, injectfullstory, injectga, injectgtm, injectwebfontloader, lintjs, vendorsjs, frameworkjs, corejs, mainjs, combinejs, css, tempcss, criticalcss, move, zipassets, appicons, meta, a11ycheck, fingerprintassets, injectscripts, optimizeassets, robots, sitemap, clean, minify, sw, preview, completed));
...
// PARTIAL TESTING
gulp.task('dev', gulp.series(version, clear, revconfig, html, dialog, injectwebfontloader, lintjs, vendorsjs, frameworkjs, corejs, mainjs, combinejs, css, tempcss, criticalcss, move, appicons, meta, a11ycheck, fingerprintassets, injectscripts, optimizeassets, minify, clean, completed));


// SCRIPTS
gulp.task('devjs', gulp.series(clear, lintjs, vendorsjs, frameworkjs, corejs, mainjs, combinejs));
gulp.task('devcss', gulp.series(clear, css));
gulp.task('scripts', gulp.series(clear, lintjs, vendorsjs, frameworkjs, corejs, mainjs, combinejs, css));
